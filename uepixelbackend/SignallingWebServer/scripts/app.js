...
// Customization Start
const LATEST_STATUS_TEXT = "Latest Status";
...
const EXTERIOR_POINT_COMMAND = "YourWebSiteCommand4";
...
const INTERIOR_POINT_BUTTON_ID = "interior-button";
const TALK_COPILOT_BUTTON_ID = "talk-copilot-button";
const EXPLAIN_COPILOT_BUTTON_ID = "explain-copilot-button";
...
let speechTokenTimestamp = 0;
let speechRegion = "";

const WEB_API_BASE_URL = "Your Public Web API URL";
const WEB_API_SPEECH_FUNC = "azurespeech/token";
const SPEECH_TOKEN_THRESHOLD = 300000;
const WEB_API_GPT_FUNC = "azureopenai/generate";
const WEB_API_GPTV_FUNC = "openai/generate";
// Customization End
...
const logger = ApplicationInsightsLogger();
...
function log(str) {
  ...
  logger.logInfo(`${Math.floor(Date.now() - t0)}: ` + str);
}
...
function gamepadConnectHandler(e) {
  ...
  logger.logInfo("Gamepad connect handler");
  gamepad = e.gamepad;
  ...
  logger.logInfo("gamepad: " + gamepad.id + " connected");
  rAF(updateStatus);
}

function gamepadDisconnectHandler(e) {
  console.log("Gamepad disconnect handler");
  logger.logInfo("Gamepad disconnect handler");
  console.log("gamepad: " + e.gamepad.id + " disconnected");
  logger.logInfo("gamepad: " + e.gamepad.id + " disconnected");
...
// Customization Start
function setupButtonEvent(buttonId, action) {
...
function setupCheckboxEvent(checkboxId, action) {
...
function restartExperience() {
  // Unreal command
  startUnrealCommand(RESTART_EXPERIENCE_COMMAND, RESTART_EXPERIENCE_BUTTON_ID);
}

function toggleView(event) {
  // Unreal command
  startUnrealCommand(event.target.checked ? TOGGLE_VIEW_NEW_COMMAND : TOGGLE_VIEW_OLD_COMMAND, TOGGLE_VIEW_CHECKBOX_ID);
}

function talkToCopilot() {
  // Web API call
  startWebApiCommand(TALK_COPILOT_BUTTON_ID, false);
} 

function exteriorPoint() {
  // Unreal command
  startUnrealCommand(EXTERIOR_POINT_COMMAND, EXTERIOR_POINT_BUTTON_ID);
}

function interiorPoint() {
  // Unreal command
  startUnrealCommand(INTERIOR_POINT_COMMAND, INTERIOR_POINT_BUTTON_ID);
}

function explainCopilot() {
  // Web API call
  startWebApiCommand(EXPLAIN_COPILOT_BUTTON_ID, true);
}

async function startWebApiCommand(buttonId, vision = false) {
  let button = document.getElementById(buttonId);
  if (button) {
    button.disabled = true;

    updateStatusText(WEB_API_COMMAND_TEXT);

    try {
      const { token, region } = await fetchSpeechToken();
      setSpeechToken(token, region);

      if (vision) {
        await startVisionCommand();
      } else {
        await startVoiceCommand(() => processVoiceCommand());
      }
    } catch (error) {
      console.error(`Error in Web API Command: ${error}`);
    }
  } 
}

function endWebApiCommand(status, buttonId) {
  let button = document.getElementById(buttonId);
  if (button) {
...
function startUnrealCommand(name, id) {
  let button = document.getElementById(id);
  if (button) {
    button.disabled = true;

    emitUIInteraction(name);
    updateStatusText(UNREAL_COMMAND_TEXT);
...
function updateStatusText(text) {
  ...
  if (status) {
    status.innerText = `${LATEST_STATUS_TEXT}: ${text}`;
  }
}
...
async function fetchSpeechToken() {
  try {
    const thresholdTime = SPEECH_TOKEN_THRESHOLD;
    const currentTime = new Date().getTime();
    if (speechToken && speechRegion && (currentTime - speechTokenTimestamp < thresholdTime)) {
      return { token: speechToken, region: speechRegion };
    }

    const response = await fetch(`${WEB_API_BASE_URL}/${WEB_API_SPEECH_FUNC}`, {
      method: 'POST'
    });
    ...
    speechToken = data.token;
    speechRegion = data.region;
    speechTokenTimestamp = currentTime;
    return { token: speechToken, region: speechRegion };
  } catch (error) {
    console.error(`Failed to fetch speech SDK token: ${error}`);
    throw error;
  }
}

function setSpeechToken(token, region) {
...
async function processVoiceCommand(stop = false) {
  if (!lastRecognizedText || stop) {
    console.error("No voice command to process.");
    endWebApiCommand("Error", TALK_COPILOT_BUTTON_ID);
    return;
  }

  try {
    const response = await fetch(`${WEB_API_BASE_URL}/${WEB_API_GPT_FUNC}`, {
      method: 'POST',
      headers: {
          'Content-Type': 'application/json'
      },
      body: JSON.stringify({ question: lastRecognizedText })
    });
    ...
    endVoiceCommand(text);
  } catch (error) {
    console.error(`Could not process voice command: ${error}`);
  }
}

function endVoiceCommand(text, buttonId = TALK_COPILOT_BUTTON_ID) {
  if (!speechSynthesizer || !text) {
    console.error("Speech synthesizer is not initialized or there is no text to speak.");
    return;
  }

  speechSynthesizer.speakTextAsync(text, result => {
    if (result.reason === SpeechSDK.ResultReason.SynthesizingAudioCompleted) {
      console.log(`Synthesis succeeded for text: ${text}`);
      endWebApiCommand("Success", buttonId);
      ...
      endWebApiCommand("Cancel", buttonId);
    }
    speechSynthesizer.close();
  }, err => {
    console.error(`TTS Error: ${err}`);
    endWebApiCommand("Error", buttonId);
    speechSynthesizer.close();
  });
}

async function startVisionCommand() {
  const text = await processVisionCommand();
  if (text) {
    endVisionCommand(text);
  } else {
    console.log('No text received from processVisionCommand or an error occurred.');
  }
}

async function processVisionCommand() {
  try {
    const response = await fetch(`${WEB_API_BASE_URL}/${WEB_API_GPTV_FUNC}`, {
      method: 'POST',
      headers: {
          'Content-Type': 'application/json'
      },
      body: JSON.stringify({ image: captureScreenImage() })
    });
    ...
    console.error(`Could not process vision command: ${error}`);
    return '';
  }
}

function endVisionCommand(text, buttonId = EXPLAIN_COPILOT_BUTTON_ID) {
  endVoiceCommand(text, buttonId);
}
...
function captureScreenImage() {
  ...
  if (videoElements.length === 0) {
    console.error('No video element found inside #player');
    return null;
  }
...
function playVideoStream() {
  if (webRtcPlayerObj && webRtcPlayerObj.video) {
    ...
    webRtcPlayerObj.video.play().catch(function (onRejectedReason) {
      ...
      logger.logError(onRejectedReason);
      ...
      logger.logInfo("Browser does not support autoplaying video without interaction - to resolve this we are going to show the play button overlay.");
      showPlayOverlay();
    });
    ...
    logger.logError("Could not player video stream because webRtcPlayerObj.video was not valid.");
  }
}
...
function setupWebRtcPlayer(htmlElement, config) {
  ...
  logger.initialize(config.appInsightsInstrumentationKey, config.appInsightsMetricFlushInterval);
  webRtcPlayerObj = new webRtcPlayer(config, logger);
  ...
  webRtcPlayerObj.onWebRtcCandidate = function (candidate) {
    if (ws && ws.readyState === WS_OPEN_STATE) {
      ...
      logger.logInfo(`-> SS: iceCandidate\n${JSON.stringify(candidate, undefined, 4)}`);
      ws.send(JSON.stringify({
        type: 'iceCandidate',
        candidate: candidate
      }));
  ...
  function processFreezeFrameMessage(view) {
    ...
    // No existing freeze frame jpeg, make one
      ...
      logger.logInfo(`received first chunk of freeze frame: ${freezeFrame.jpeg.length}/${freezeFrame.size}`);
    }
    ...
    if (freezeFrame.jpeg.length === freezeFrame.size) {
      ...
      logger.logInfo(`received complete freeze frame ${freezeFrame.size}`);
      showFreezeFrame();
    }
    // We received more data than the freeze frame payload message indicate (this is an error)
      ...
      logger.logError(`received bigger freeze frame than advertised: ${freezeFrame.jpeg.length}/${freezeFrame.size}`);
      freezeFrame.jpeg = undefined;
      freezeFrame.receiving = false;
    }
  }
  ...
  webRtcPlayerObj.onDataChannelMessage = function (data) {
    ...
    if (view[0] === ToClientMessageType.QualityControlOwnership) {
      ...
      logger.logInfo("Received quality controller message, will control quality: " + ownership);
      ...
      logger.logInfo(commandAsString);
      ...
      logger.logInfo("Got latency timings from UE.");
      console.log(latencyTimingsAsString);
      logger.logInfo(latencyTimingsAsString);
      ...
      logger.logError(`unrecognized data received, packet ID ${view[0]}`);
    }
  };
...
function onWebRtcAnswer(webRTCData) {
  ...
  webRtcPlayerObj.onAggregatedStats = (aggregatedStats) => {
    // further aggregate stats in the logger to be sent to appInsights. 
    logger.metricsAggregator(aggregatedStats);
...
function updateVideoStreamSize() {
  ...
  if (now - lastTimeResized > 1000) {
    ...
    logger.logInfo(descriptor);
    lastTimeResized = new Date().getTime();
  } else {
    console.log('Resizing too often - skipping');
    logger.logInfo('Resizing too often - skipping');
...
function registerLockedMouseEvents(playerElement) {
  ...
  function lockStateChange() {
    if (document.pointerLockElement === playerElement ||
      ...
      logger.logInfo('Pointer locked');
      document.addEventListener("mousemove", updatePosition, false);
    } else {
      console.log('The pointer lock status is now unlocked');
      logger.logInfo('The pointer lock status is now unlocked');
...
function registerTouchEvents(playerElement) {
  ...
  function rememberTouch(touch) {
    ...
    if (finger === undefined) {
      ...
      logger.logInfo('exhausted touch indentifiers');
    }
...
async function handleUnrealResponse(data) {
  ...
  const command = jsonData.unrealOutput?.command;
  if (!command) {
    console.error('No command found in JSON data');
    return;
  }
...
function registerUnrealEvents() {
  addResponseEventListener("handle_responses", handleUnrealResponse);
}
// Customization End
...
function connect() {
  ...
  ws.onmessage = function (event) {
    ...
    if (msg.type === 'config') {
      ...
      logger.logInfo(`invalid SS message type: ${msg.type}`);
    }
  };

  ws.onerror = function (event) {
    console.log(`WS error: ${JSON.stringify(event)}`);
    logger.logInfo(`WS error: ${JSON.stringify(event)}`);
  };

  ws.onclose = function (event) {
    console.log(`WS closed: ${JSON.stringify(event.code)} - ${event.reason}`);
    logger.logInfo(`WS closed: ${JSON.stringify(event.code)} - ${event.reason}`);
    ws = undefined;
...
function onConfig(config) {
  ...
  switch (inputOptions.controlScheme) {
    ...
    default:
      ...
      logger.logInfo(`ERROR: Unknown control scheme ${inputOptions.controlScheme}`);
      registerLockedMouseEvents(playerElement);
      break;
  }
}
...
function load() {
  setupHtmlEvents();
  // Customization Start
  setupHtmlEventsExperience();
  // Customization End
  setupFreezeFrameOverlay();
  registerKeyboardEvents();
  // Customization Start
  registerUnrealEvents();
  // Customization End
  start();
}
